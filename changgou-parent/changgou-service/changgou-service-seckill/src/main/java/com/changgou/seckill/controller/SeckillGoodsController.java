...
public class SeckillGoodsController {
    ...
    public Result<SeckillGoods> one(String time,Long id){
        //调用Service查询商品详情
        return new Result<>(true, StatusCode.OK, "查询秒杀商品详情成功", seckillGoodsService.one(time,id));
    }
    ...
    public Result<List<Date>> dateMenus(){
        return new Result<>(true, StatusCode.OK, "查询秒杀时间菜单成功",DateUtil.getDateMenus());
    }
    ...
    public Result<List<SeckillGoods>> list(String time){
        //调用Service查询数据
        return new Result<>(true, StatusCode.OK, "秒杀商品列表查询成功", seckillGoodsService.list(time));
    }
